...
Browser acceptance does not currently run. The app declares eleven browser scenarios and ten of them have never existed, because a generated Toolcraft app cannot launch its browser with a GL backend able to render this scene: headless Chrome's default backend kills the page while the transmission sampler warms up, product tests may not import Playwright directly, and `playwright.config.ts` is signed. Closing that needs the shared Toolcraft configuration changed and the app regenerated. `npm run verify:delivery` and `npm run verify:perf` are blocked behind the same gap. The reasoning and the measurements are in the worklog.
