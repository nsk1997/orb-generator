...
- Shared-response proof: `src/app/orb/orb-code-snippet.test.ts` requires every entry in `orbResponse` to be emitted as data *and* read back by name in the snippet, so a coefficient that drifts, is emitted but unused, or is used but not emitted fails. `src/app/orb/orb-response.test.ts` pins the behaviour each coefficient buys — bloom rising with glow and lifting on the transient lead, a lit floor under the interior at glow 0, the halo tracking the glow control, and the core churning inside a still shell, which is what Think is.
- Behaviour-preserving proof: the app-versus-snippet capture was rerun after the extraction and is unchanged — Glass luminance x1.01 and saturation x1.01, Aurora x1.01 and x1.07, against x1.01/x1.01 and x1.01/x1.06 measured before it. The emitted snippet rendered in the standalone app with no errors.
- Acceptance-claim proof: flipping `orb.style` to `browser: false` fails `app-acceptance.base-coverage`, `app-acceptance.base-coverage` section inventory, and `app-schema` with "Style / style (orb.style) must have browser acceptance coverage." Browser coverage is mandatory for every visible schema control, so the declaration cannot be softened to match reality.

- Resting-orb proof: mean absolute channel difference between two consecutive captures of the product output, 50ms apart, at `Flow speed` 0. Before: Glass 0.3153 with 10.64% of channels changed, Obsidian 0.0028 with 0.24% changed. After: Obsidian 0.0000 with a maximum channel delta of 0 and 0.00% of channels changed — byte-identical frames, which is what the acceptance gate's stable baseline requires and had never been reachable. Glass improved to 0.2960 but does not rest, which is the transmission sampler rather than the product clock. A fully flowing Glass measures 3.68 for scale.
- Clock-ownership proof: `src/app/orb/orb-materials.test.ts` writes a wall-clock value to the uniform exactly as Drei does each frame and requires the product phase to be reported back instead, and requires a material with no `time` uniform to fail loudly rather than silently keep library time.
- Damping proof: `src/app/orb/orb-damping.test.ts` runs 600 frames of both implementations toward 0 and requires the plain damp to still be non-zero while the settled one is exactly 0, requires the eased value mid-flight to remain identical to plain damping so the change is arrival and not snapping, and proves the same arrival for the colour chase.
- Browser-acceptance blocker proof: ten of the eleven declared browser tests do not exist anywhere under `e2e/`. Under headless Chrome's default GL backend a minimal spec dies in `createToolcraftBrowserProofSession` with "Internal server error, session closed"; with ANGLE over SwiftShader the same spec ran for 10.1 minutes across the style loop without crashing. Applying that flag from product code fails `ai:check` with "Direct Playwright runtime authority is not available to product code", and applying it in `playwright.config.ts` fails the integrity check as a modified framework-owned file. Both refusals were reproduced directly rather than inferred.

- App-versus-snippet proof for this iteration: each style was rendered in the app and in its own copied snippet at the same 480px canvas, camera distance 8 and `#0A0A12` background, and compared on mean luminance and on mean saturation of lit pixels. Before: Glass luminance x0.77 and saturation x1.06, Aurora x0.88 and x1.17, Nebula x0.64 and x1.10, Plasma x0.94 and x1.22 — the snippet dimmer and more saturated on every style, in the same direction each time. After: Glass x1.01 and x1.01, Aurora x1.01 and x1.06, Nebula x0.91 and x1.06, Plasma x1.06 and x0.98 — residuals now scatter either side of 1.0 rather than leaning one way. Captured at `.toolcraft/browser-artifacts/exposure-app-<style>-idle.png` and `exposure-snippet-<style>-idle.png`.
- Background proof: a radial luminance profile in twelve rings showed the app and the snippet differing by a constant 0.041 at every radius outside the orb, which is exactly the luminance of `#0A0A12`; the halo itself measured identical in both. The Nebula snippet's corner pixel read `#000000` against the app's `#0A0A12` before the fix and `#0A0A12` after. Painting the canvas element's own CSS background red showed red in neither, which is what established that the app clears the background inside WebGL rather than sitting on a CSS colour.
- Colour-space proof: the app's tint and the snippet's old tint were computed side by side for four styles — Aurora `#E9ECFF` against `#DBE5FF`, Glass `#D1CBFF` against `#C4B6FF`, Plasma `#FF9CB8` against `#FF77AE`, Nebula `#F4EFFF` against `#F1E7FF` — the app lighter and roughly two-thirds the saturation in each case, for the same authored `tintLift`. `src/app/orb/orb-code-snippet.test.ts` now requires every style's emitted `glassTint` to equal `orbGlassTintHex`, and separately requires that value to differ from an sRGB lift, so the first test cannot pass vacuously if the two spaces ever agree. A third test requires every style's snippet to emit the clear-colour call.
...
- Performance intent: ordinary-product-work
- Verification: `npm run verify:delivery`.
- Risks: Risk: the motion has not been judged by a human. It was verified to run, to morph, and to settle, not to feel better than the damping it replaced; the timings and eases are one-line changes in `orbTransitionMorph` and each style's `motion` block. Risk: an unknown GSAP ease name resolves silently to the default curve, so the signature tests assert on the resolved curve rather than the name — but a *valid* ease that is simply wrong for a style is still invisible to them. Risk: the snippet's determinism is proven by test, not exercised by the product, because this app has no video export to need it. Risk: browser acceptance coverage for this batch was attempted and not landed. A Playwright spec asserting that a state change is still mid-transition 150ms after the click passed once, in 4.1 minutes, then wedged the browser on a later run — `page.waitForTimeout` itself exceeded 120s. Under headless software GL the orb's 1024px transmission sampler is heavy enough that the page stops responding, and it is worse at the closest camera distance, where the orb fills the frame. A second spec could not resize the canvas at all: editing the width and height fields destroys the execution context. The spec was removed rather than shipped flaky, so the motion and framing changes in iterations 12 and 13 rest on unit tests, on measurements taken from real 4K exports, and on manual browser runs against the dev server — not on repeatable browser evidence.
...
### Iteration 14 — The copied orb matches the app, and the divergence was the other way round

- Request: "start picking by priority wise", against a standing list in which the app-versus-snippet exposure divergence was the highest-value open item because Copy Code is the product's output.
- Task type: Export/copy fidelity, renderer, acceptance.
- User-visible result: A pasted orb now renders the same as the one on screen. Two separate faults were found and fixed: every style's glass tint was computed in a different colour space in the snippet than in the app, and the snippet painted the scene background in CSS where the app clears it inside WebGL, which erased the background entirely on any style whose halo reaches the frame edge.
- Source/reference checked: Iteration 11's recorded risk, which said "the copied snippet renders the bands noticeably hotter than the app does, so its hues wash toward white". Measurement showed that claim is backwards and now stale: at the time it was written the snippet had the Iteration 12 environment-probe and sampler-resolution faults, and after those were fixed nobody re-measured. The snippet was in fact *dimmer* and *more* saturated than the app on every style tested.
- Reference inputs: None.
- Docs/contracts read: `AGENTS.md`, `docs/toolcraft/workflow.md`, `docs/toolcraft/decision-contract.md`, `docs/toolcraft/core/runtime-boundary.md`, `docs/toolcraft/renderer-technique.md`, `docs/toolcraft/component-rules.md`.
- Contract rules applied: `output-export-required`, `renderer-technique-inventory`, `acceptance-product-observable`.
- View interaction intent: Unchanged; orbit with `view.orbit`.
- Interaction ownership: Unchanged.
- Decision: The reproduction came before the diagnosis, and it is the only reason either fault was found. Reading the two sources side by side was actively misleading — the bloom formula, the core-intensity formula, the halo shader, the light cards, the tone mapping and the transmission material's props are all identical between them, so the files agree everywhere a reader would think to look. What the files do not show is that `Color.lerp` runs in three's linear working space while lerping the sRGB hex digits does not, so the same authored `tintLift` produced two different colours: the app's Aurora tint is `#E9ECFF` at saturation 0.086 against the snippet's `#DBE5FF` at 0.141. The tint now has one definition, `applyOrbGlassTint` in `orb-materials.ts`, which the render loop calls per frame on the damped colour and the emitter calls once to produce a hex, so the app and the snippet cannot drift again without the shared function moving. The second fault is that a CSS background is not equivalent to a cleared one: the halo is additive and writes alpha 1, so wherever the halo plane covers the frame it turns the canvas opaque and masks whatever CSS sits behind it. The emitted Canvas now calls `gl.setClearColor` in `onCreated`, which is what the app's own clear-colour effect does.
- Alternatives rejected: Changing the app to lerp in sRGB, which would have been the cheaper edit but is the wrong direction — every style's `tintLift` was authored by eye against the app, so the app is the reference and the snippet is what must match it. Leaving the CSS background in place and enlarging nothing, on the argument that Glass looked correct: Glass looked correct only because its 4.2 halo does not reach the corners, and Nebula's 4.6 halo does. Keeping `lightenTowardWhite` and merely correcting its arithmetic, which would have left two implementations of one idea.
- State/output mapping: `applyOrbGlassTint(out, primary, tintLift)` is called from the render loop in `orb-scene.tsx` onto `body.color`, and `orbGlassTintHex` serialises the same value into the snippet's `ORB.glassTint`. The emitted `<Canvas>` gains `alpha: true` and an `onCreated` clear-colour call carrying `options.backgroundColor`.
- Performance intent: ordinary-product-work
- Verification: `npm run verify:delivery`.
- Checks actually run: later feature work, so focused checks rather than the delivery gate — `npm run typecheck`, `npx vitest run src` (512 passing, +3 for this batch), `npm run ai:check`, `node scripts/check-toolcraft-integrity.mjs`, `npm run docs:check`, `npm run build`, plus the browser measurements under Evidence. `npm run test:feature -- actions.output` cannot run: the acceptance entry names a browser test "browser: Export PNG downloads orb pixels and Copy Code writes a snippet" that has never existed in the Playwright suite. This is the same pre-existing gap already recorded for `orb.style`, it covers both of this app's output actions, and no file under `e2e/` or `src/app/app-acceptance-data.ts` was touched in this batch.
- Risks: Risk: the residual difference after both fixes is a mean luminance ratio between 0.91 and 1.06 depending on style, which sits inside the app's own frame-to-frame variation — three app-only Nebula captures measured 0.0995, 0.1058 and 0.1082 — so it is read as capture phase rather than as a third fault, but it is not proven to be nothing. Risk: the fix is proven at one canvas size, one camera distance and the default background; a background whose luminance is far from `#0A0A12` was not measured. Risk: the harness that found this is gitignored, so the comparison is repeatable only on this machine. Risk: `.toolcraft/emit-snippet.mjs` had been stale since Iteration 12 — it still passed the removed `form` option — which means nothing had emitted a snippet through that path since the option changed, and the Iteration 12 snippet evidence was gathered before that rename landed.

### Iteration 15 — The orb can now come to rest, and why browser acceptance still cannot run here

- Request: "start picking by priority wise" and then "ok" to the recommendation that the renderer should own its clock so `Flow speed` at 0 genuinely freezes the orb, which would unblock the missing browser acceptance.
- Task type: Renderer time ownership, acceptance infrastructure.
- User-visible result: `Flow speed` at 0 now stops the orb instead of merely slowing it, and every damped control settles on the exact value the panel shows rather than approaching it forever. An opaque style at rest is now byte-for-byte identical between frames, measured at 0.0000 mean channel difference against 0.3153 before.
- Source/reference checked: `node_modules/@react-three/drei/core/MeshTransmissionMaterial.js` line 353, which assigns `material.uniforms.time.value = state.clock.elapsedTime` on every frame and does so *before* it checks transmission, so opaque styles get it too. The app had always written `body.time = state.flowPhase` immediately before, and that write was discarded every frame.
- Reference inputs: None.
- Docs/contracts read: `AGENTS.md`, `docs/toolcraft/workflow.md`, `docs/toolcraft/acceptance-testing.md`, `docs/toolcraft/decision-contract.md`, `docs/toolcraft/core/runtime-boundary.md`, `docs/toolcraft/renderer-technique.md`, `docs/toolcraft/component-rules.md`.
- Contract rules applied: `renderer-technique-inventory`, `acceptance-product-observable`.
- View interaction intent: Unchanged; orbit with `view.orbit`.
- Interaction ownership: Unchanged.
- Decision: Two independent reasons the orb never rested, both found by measuring rather than reading. Drei owned the material's `time` uniform, so the standing risk that "the material's internal distortion is not phase-integrated" was not a subtlety about phase but a control that did not reach its own renderer: `Flow speed` could not slow the surface at all. Rather than fork the material, the product redefines the uniform's accessor, which makes the library's assignment the no-op and the product the single owner of the clock. Separately, `MathUtils.damp` approaches asymptotically and never arrives, so a slider dragged to 0 left about 1e-9 behind and the phase kept creeping; damping now closes the last sliver outright, because a control set to its minimum should mean that minimum. Neither change is test scaffolding — both are the control doing what the panel says it does — but together they are also what makes a deterministic frame possible at all.
- Alternatives rejected: Forking or patching Drei's material, which puts a library copy in product source for one line. Adding a separate "pause" control, which is a second way to say `Flow speed 0`. Enabling the Toolcraft timeline to own the phase, which adds a whole panel the product deliberately does not have, and which was the fallback if owning the uniform had not been enough.
- State/output mapping: `attachOrbProductTime` is installed on every body material as it mounts, reading `shown.current.flowPhase`; a style switch mounts a new material and re-claims it. `dampSettled` and `lerpSettled` replace `MathUtils.damp` and `Color.lerp` for every damped scalar and both damped colours in the render loop.
- Performance intent: ordinary-product-work
- Verification: `npm run verify:delivery`.
- Checks actually run: `npm run typecheck`, `npx vitest run src` (517 passing, +5), `npm run ai:check`, `node scripts/check-toolcraft-integrity.mjs`, `npm run docs:check`, `npm run build`, plus the frame-difference measurements under Evidence.
- Browser acceptance was attempted again and is still not here, but the reason is now precise rather than "it wedges". Three findings. First, the app declares eleven browser tests and ten of them have never existed; only the persistence one does, so this was never a gap about iterations 12 and 13. Second, the wedge is the GL backend: headless Chrome's default backend kills the page with "Internal server error, session closed" while the transmission sampler warms up, and launching with ANGLE over SwiftShader instead ran a full eleven-style spec for ten minutes without crashing. Third, and decisively, a generated app may not apply that flag. Setting it needs either `@playwright/test` imported into a product spec, which `ai:check` rejects with "Direct Playwright runtime authority is not available to product code", or an edit to `playwright.config.ts`, which the integrity manifest rejects as framework-owned verification infrastructure. So browser acceptance for a WebGL product of this weight is not reachable from inside the generated app; it needs the shared Toolcraft Playwright configuration to launch with a GL backend that can render it, and a regenerated app. That is the documented path for a wrong shared behaviour, and it is not a product-side edit.
- Risks: Risk: the transmissive styles still do not rest exactly — Glass at flow 0 measures 0.2960 mean channel difference against 0.0000 for Obsidian — because Drei's transmission sampler renders multi-pass buffers whose results vary between frames under software GL. A stable baseline is therefore only available on the opaque styles, which is enough to anchor a comparison but is not the same as a deterministic renderer. Risk: `attachOrbProductTime` redefines a property on a library-owned uniform object, so a future Drei that stops routing through `uniforms.time.value` would silently return the orb to wall-clock time; the unit test pins the behaviour but cannot pin Drei's internals. Risk: the settling epsilon of 1e-4 is below visible precision for the current control ranges, but a future control with a much smaller domain could snap perceptibly. Risk: the ten-minute spec run failed on a select click that needs `click({ force: true })` against a busy WebGL frame, so even with the GL flag the browser suite will need that treatment throughout; this was learned but not landed.

### Iteration 16 — One definition for how the controls turn into what gets drawn

- Request: "what do you suggest?" then "yes start on #2", where #2 was to stop the acceptance data claiming browser evidence it does not have; that turned out to be forbidden, and the accepted replacement was to remove the duplication that caused Iteration 14's bug in the first place.
- Task type: Renderer/export shared definition, acceptance.
- User-visible result: No visible change, by design. Five expressions that decided what the renderer draws — shell distortion, core distortion, core intensity, halo intensity, and bloom intensity — existed twice, once in the render loop and once as copy-pasted text in the snippet emitter. They now have one definition, and Copy Code serialises the coefficients rather than restating them.
- Source/reference checked: `src/app/acceptance/control-acceptance-coverage.ts`, which is what closed off the requested task. Setting `browser: false` on `orb.style` fails three suites with "Style / style (orb.style) must have browser acceptance coverage": every visible schema control is *required* to declare browser coverage. The acceptance data therefore cannot stop claiming it, and the app stays non-compliant until the tests exist, which Iteration 15 established is blocked outside this repo. This was reproduced and reverted rather than argued.
- Reference inputs: None.
- Docs/contracts read: `AGENTS.md`, `docs/toolcraft/workflow.md`, `docs/toolcraft/decision-contract.md`, `docs/toolcraft/renderer-technique.md`, `docs/toolcraft/acceptance-testing.md`.
- Contract rules applied: `renderer-technique-inventory`, `output-export-required`, `acceptance-product-observable`.
- View interaction intent: Unchanged; orbit with `view.orbit`.
- Interaction ownership: Unchanged.
- Decision: Iteration 14's glass-tint bug was not really about colour space; it was about the same quantity being computed in two places with nothing keeping them in step, which is invisible until someone renders both and compares pixels. Five more instances of that exact shape were still present, and bloom and core intensity were duplicated three ways because the emitter writes them at both initialisation and per frame. The fix follows the pattern Iteration 12 already proved for motion timings: the coefficients live once as data in `orb-response.ts`, the render loop computes through small named functions, and the emitter interpolates the same data into a `RESPONSE` block the snippet reads by name. A coefficient can no longer be retuned on one side alone.
- Alternatives rejected: Leaving the duplicates because they currently agree, which is precisely the state Iteration 11 was in when it recorded a risk that had already stopped being true. Having the snippet import a shared module, which is impossible — the snippet is standalone source text a user pastes elsewhere, so the emitter has to inline; serialising the data is the closest thing to a shared import. Extracting the GLSL constants too, which are a different kind of duplication and were left alone.
- State/output mapping: `orbResponse` is the single data block. `orbBloomIntensity`, `orbCoreIntensity`, `orbGlowIntensity`, `orbShellDistortion`, and `orbCoreDistortion` are called from the render loop in `orb-scene.tsx`; `createOrbCodeSnippet` serialises `orbResponse` into the emitted `RESPONSE` constant and the emitted loop reads every coefficient from it.
- Performance intent: ordinary-product-work
- Verification: `npm run verify:delivery`.
